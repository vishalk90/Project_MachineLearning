...
def readfile(x):
    ...
    try:
        ...
        print ("#"*80)
        print(" Improper Syntax: try - python3 SNP.py traindata trainlabel testdata")
        print ("#"*80)
        sys.exit()
...
def getFetures(testdata,feature_cols):
    ...
    for rows in range(len(testdata)):
        ...
        for cols in range(len(testdata[0])):
            if(k<=13 and feature_cols[k] == cols):
    ...
    for i in range(0, len(bestfeature_testdata),1):
        for j in range(0, len(bestfeature_testdata[i]),1):
            bestfeaturestestdata_file.write(str(int(bestfeature_testdata[i][j]))+" ")
...
def predictLabels(traindata, trainlabels, testdata):
    ...
    for i in range(0, len(testdata), 1):
        #print(str(int(predictedlabels[i]))+" "+str(i))
        predictedlabels_file.write(str(int(predictedlabels[i]))+" "+str(i)+'\n')
